...
test("loads the configured GA4 Google tag without changing the instrument UI", async ({
  page,
}, testInfo) => {
  ...
  if (testInfo.project.name === "mobile-portrait-chromium") {
    await expect(page.getByRole("region", { name: "Landscape orientation required" })).toBeVisible()
  } else {
    await expect(
      page.getByRole("heading", { level: 1, name: "webpiano Online piano" }),
    ).toBeVisible()
