...
- Immediate standard-piano rendering, explicit `32 notes · 37 keys` counts, one 88-key navigator with a draggable STANDARD gauge, global `←` / `→` semitone movement, no range-stepper controls over the piano, and one fixed-width below-header transposition readout with visible Space pedal and arrow-key guides that follow the active Single or Dual ranges. The pedal Badge distinguishes outlined OFF from filled ON and identifies Lock versus Phone sources without shifting neighboring guidance; the same surface retains a stable count marker across natural/sharp ranges, keyboard/pointer input, sustain, and stuck-note cleanup.
