...
- Immediate standard-piano rendering, explicit `32 notes · 37 keys` counts, one 88-key navigator with a draggable STANDARD gauge, button and `←` / `→` semitone movement, one fixed-width header transposition readout that follows the active Single or Dual ranges without adding UI above the piano, a stable count marker across natural/sharp ranges, keyboard/pointer input, sustain, and stuck-note cleanup.
