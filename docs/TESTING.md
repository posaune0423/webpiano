...
- Immediate standard-piano rendering, explicit `32 notes · 37 keys` counts, one 88-key navigator with a draggable STANDARD gauge, button and `←` / `→` semitone movement, a fixed-width `KEY C` transposition label that follows each independent range, a stable count marker across natural/sharp ranges, keyboard/pointer input, sustain, and stuck-note cleanup.
