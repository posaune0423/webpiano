...
- Immediate standard-piano rendering, explicit `32 notes · 37 keys` counts, one 88-key navigator with a draggable STANDARD gauge, global `←` / `→` semitone movement, no range-stepper controls over the piano, and one fixed-width below-header transposition readout with a visible arrow-key guide that follows the active Single or Dual ranges. The Pedal menu—not the status strip—contains the `[SPACE]` shortcut and a fixed-width Badge that distinguishes outlined OFF from filled ON and identifies Lock, Phone, and combined Phone + Lock sources; the same surface retains a stable count marker across natural/sharp ranges, keyboard/pointer input, sustain, and stuck-note cleanup.
