...
- A slim status strip immediately below the header carries one fixed-width transposition readout and the arrow-key guide, leaving the navigator and playable piano free of extra status UI. It shows `KEY C` with `← → MOVE RANGE` in Single mode and a compact `L C · U C` with `SELECT RANGE · ← → FINE MOVE` in Dual mode; `KEY` describes chromatic transposition and never claims a major or minor mode. Pedal state and the Space shortcut stay inside the header Pedal menu: its fixed-width Badge is outlined as `PEDAL OFF`, fills as `PEDAL ON · LOCK` for Sustain Lock, fills as `PEDAL ON · PHONE` while the remote phone pedal is down, and reports `PEDAL ON · PHONE + LOCK` when both sources are active, while the Sustain Lock item shows `[SPACE]`.
...
- The unobtrusive below-header transposition/arrow strip and piano surface without range steppers are recorded in `transposition-key-standard-desktop.png`, `transposition-key-dual-desktop.png`, and `transposition-key-standard-mobile-landscape.png`; the Pedal menu records OFF and `[SPACE]` in `pedal-lock-states-desktop.png` and the filled remote-pedal state in `pedal-phone-active-desktop.png`.
